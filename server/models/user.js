...
var conString = "postgres://w_admin:11111@localhost:5432/voteapp";
...
var User = function (data){
    ...
    this.password= data.password; //need to declare the things that i want to be remembered for each user in the database
    ...
    this.save = function(callback) {
        ...
        console.log(this.email +' will be saved');
            ...
            client.query('INSERT INTO people(email, password) VALUES($1, $2)', [this.email, this.password], function (err, result) {
                if(err){
                    console.log(err);
            ...
            client.query('SELECT * FROM people ORDER BY u_id desc limit 1', null, function(err, result){
...
User.findOne = function(email, callback){
    ...
    client.query("SELECT * from people where email=$1", [email], function(err, result){
        if(err){
